 ...
 *   VirtualSerial — 单向串口通信（依次发送豆子包 + 数字包）
 *   visualize     — 绘制检测框
j * 通信流程（单向双包）：
 *   扫描豆子(30帧) → 发送豆子包 → 间隔(15帧) → 扫描数字(30帧) → 发送数字包 → 结束
 *
 * 混合帧处理：豆子阶段自动过滤数字，数字阶段自动过滤豆子（不再作废整帧）
 * 推理补全：≥2 个豆子 → 推理第 3 个；≥4 个数字 → 推理第 5 个
 *
...
enum class CommPhase {
    SCANNING_BEANS,    // 扫描豆子帧（最多 30 帧）— 第一阶段
    BEAN_GAP,          // 豆子包已发，等待 15 帧间隔
    SCANNING_DIGITS,   // 扫描数字帧（最多 30 帧）— 第三阶段
    DONE               // 双包已发，通信结束
};

static constexpr int DIGIT_SCAN_FRAMES = 300;
static constexpr int BEAN_SCAN_FRAMES  = 30;
static constexpr int GAP_FRAMES        = 15;

// ---- 显示开关（树莓派上设为 false 可显著提升帧率） ----
static constexpr bool SHOW_INFO_BAR   = true;   // 顶部信息栏（FPS/阶段/包状态）
static constexpr bool SHOW_DETECTIONS = true;   // 检测框 + 类别标签
static constexpr bool SHOW_CENTERS    = true;   // 中心点 + 序号
...
/// 从检测结果中过滤出只含豆子的项（class_id ∈ {0,1,2}）
static std::vector<Detection> filterBeans(const std::vector<Detection>& dets) {
    std::vector<Detection> result;
    for (const auto& d : dets)
        if (isBeanClass(d.class_id)) result.push_back(d);
    return result;
}

/// 从检测结果中过滤出只含数字的项（class_id ∈ {3,4,5,6,7}）
static std::vector<Detection> filterDigits(const std::vector<Detection>& dets) {
    std::vector<Detection> result;
    for (const auto& d : dets)
        if (isDigitClass(d.class_id)) result.push_back(d);
    return result;
}
...
static const char* phaseName(CommPhase phase) {
    switch (phase) {
        ...
        case CommPhase::BEAN_GAP:        return "GAP_WAIT";
...
int main(int argc, char** argv) {
    ...
    // 4. 通信状态机初始化
    // ============================================================
    CommPhase phase = CommPhase::SCANNING_BEANS;
    int frameCounter = 0;       // 当前阶段帧计数
    bool beanSent    = false;
    bool digitSent   = false;

    printf("\n[COMM] 通信流程开始\n");
    printf("[COMM] Phase 1: 扫描豆子 (%d 帧) → 发送豆子包\n", BEAN_SCAN_FRAMES);
    printf("[COMM] Phase 2: 间隔等待 (%d 帧)\n", GAP_FRAMES);
    printf("[COMM] Phase 3: 扫描数字 (%d 帧) → 发送数字包\n", DIGIT_SCAN_FRAMES);
    ...
    while (cam.read(frame)) {
        ...
        case CommPhase::SCANNING_BEANS: {
            // 过滤出豆子（忽略同帧中的数字），重新排序 + 去重
            auto beanDets = filterBeans(sorted);
            auto beanUniqueIds = extractUniqueClassIds(beanDets);

            if (beanUniqueIds.size() >= 2) {
                // 取前 2 个实际检测到的豆子（按 X 排序）
                std::vector<int> beanIds(beanUniqueIds.begin(),
                                         beanUniqueIds.begin() + std::min<size_t>(2, beanUniqueIds.size()));
        ...
        case CommPhase::BEAN_GAP: {
            frameCounter++;
            if (frameCounter >= GAP_FRAMES) {
                printf("[COMM] → 间隔结束，开始扫描数字\n");
                phase = CommPhase::SCANNING_DIGITS;
                frameCounter = 0;
            }
            break;
        }
        ...
        case CommPhase::SCANNING_DIGITS: {
            // 过滤出数字（忽略同帧中的豆子），重新排序 + 去重
            auto digitDets = filterDigits(sorted);
            auto digitUniqueIds = extractUniqueClassIds(digitDets);

            if (digitUniqueIds.size() >= 4) {
                // 取前 4 个实际检测到的数字（按 X 排序）
                std::vector<int> digitIds(digitUniqueIds.begin(),
                                          digitUniqueIds.begin() + std::min<size_t>(4, digitUniqueIds.size()));
        ...
        // ---------- 5d. 绘制（可通过 config.yaml 关闭以提升帧率） ----------
        if (!sorted.empty()) {
            if (SHOW_DETECTIONS) drawDetections(display, sorted);
            if (SHOW_CENTERS)    drawCenters(display, centers);
        }
        ...
        // ---------- 5f. 顶部信息栏（可通过 config.yaml 关闭以提升帧率） ----------
        if (SHOW_INFO_BAR) {
            // 行 1: 阶段 + 帧计数 + 状态
            char line1[160];
            ...
            // 混合帧提示（豆子+数字同框时，当前阶段自动过滤非目标类别）
            if (!sorted.empty() && !isPureBeans(sorted) && !isPureDigits(sorted)) {
                const char* filterHint = (phase == CommPhase::SCANNING_BEANS)  ? "MIXED (digits filtered out)"
                                       : (phase == CommPhase::SCANNING_DIGITS) ? "MIXED (beans filtered out)"
                                       : "MIXED";
                cv::putText(display, filterHint,
                            cv::Point(10, 72),
