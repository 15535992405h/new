...
bool Config::loadFromYAML(const std::string& yamlPath)
{
    ...
    if (!fs["display"].empty()) {
        ...
        CFG_READ(dsp, show_info_bar,        showInfoBar);
        CFG_READ(dsp, show_detections,      showDetections);
        CFG_READ(dsp, show_centers,         showCenters);
    }
...
void Config::print() const
{
    printf("\n"
           ...
           windowName.c_str(),
           fpsInterval,
           showInfoBar    ? "true" : "false",
           showDetections ? "true" : "false",
           showCenters    ? "true" : "false");
}
